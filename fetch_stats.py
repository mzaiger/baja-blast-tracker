...
def get_savant_bombs(min_distance=MIN_DISTANCE):
    ...
    for row in reader:
        ...
        if game_date == today:
            continue  # live feed handles today
...
def get_live_bombs(min_distance=MIN_DISTANCE):
    ...
    # No gameType param — fetch all, filter to R ourselves so we can log what comes back
    ...
    for pk in game_pks:
        ...
        for play in plays:
            ...
            event_name = result.get("event", "").lower()

            if "home_run" not in event_type and "home run" not in event_name:
                continue
